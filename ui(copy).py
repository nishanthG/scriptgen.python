...
class Dynamic_Gui():
	...
	def adjust_window_position(self, width, height, window_name):
		...
		# window_name.resizable(width=0,height=0)
	...
	def GUI_Maker(self):
		...
		widget_.pack(side="left")
  		...
  		createWidgetButton_ = Button(sw.window, width=20,text="Reset",command=self.Clear)
		...
		widget_.pack(side="left")
	  	...
	  	createWidgetButton_3 = Button(sw.window, width=20,text="Generate Script", command=self.Generate_Script)
	...
	def add_Element(self):
		...
		Button(frame,width=10,text="Submit",command=self.new_info_Element).grid(row=2,column=5,padx=10)
		root.mainloop()
	
	def new_info_Element(self):
	...
	def delete_Element(self):
		...
		button=Button(root_,width=10,text="Delete",command=self.delete_element_info)
		button.place(x=160,y=35)
		root_.mainloop()

	
	def delete_element_info(self):
		file1_content = open(self.filename,'r').read().splitlines()
